...
excerpt: "express, routers, templates, mongoDB and mongoose, user authentication"
...
1. 서버를 생성한 후에는 서버가 사용자 요청에 응답하도록 해야한다.
2. 사용자는 HTTP 프로토콜을 사용하여 요청하는데. 이 요청을 GET 요청이라고 한다.
3. 주소 표시줄에 URL을 입력하고 페이지가 로드되면 실제로 서버에 GET 요청을 보내고 응답을 받고 브라우저에 응답을 표시한다.



## #3.2 GET Requests part Two

1. 서버가 사용자 브라우저의 GET 요청에 응답하도록 하기 위해 아래와 같이 "홈 URL"과 "eventHandler 함수"를 받는 .get()을 설정합니다.
   const handleHome = () => console.log("홈화면으로 이동");
   app.get("/", handleHome);

2. 이렇게되면 console에는 값이 나오지만 응답(res)을 안하므로 브라우져는 계속 기다리고만 있음
...
```
// 미들웨어 "one"으로 시작하지만 next()가 호출됐기 때문에 컨트롤러 "three"에서 끝
// 컨트롤러 "three"에 가기전에 one, two 에서 코드를 작성할수 있다 
...
const three = (req,res) => {
console.log("이제 3개를 처리 중입니다.")
}

// app.get은 "one" 핸들러로 "/" URL을 방문하는 사용자를 처리함.
app.get("/", one, two, three)
```



## #3.8~3.10 server, controlleres, middleware Recap
